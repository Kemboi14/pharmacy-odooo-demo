...
def post_init_hook(cr, registry):
    ...
    for config in env['pos.config'].search([('is_pharmacy_pos', '=', True)]):
        if payment_methods:
            config.write({'payment_method_ids': [(6, 0, [pm.id for pm in payment_methods])]})
    ...
    if westlands_branch:
        pos_config = env['pos.config'].search([
            ('is_pharmacy_pos', '=', True),
            ('branch_id', '=', westlands_branch.id)
        ], limit=1)
        
        if pos_config and demo_products['paracetamol'] and demo_lots['para']:
            _ensure_demo_pos_order('PHARM-DEMO-CASH-001', pos_config, is_insurance=False)
            _ensure_demo_pos_order('PHARM-DEMO-INS-001', pos_config, is_insurance=True)
    else:
        _logger.warning("Westlands branch (WST002) not found. Skipping demo POS order creation.")
