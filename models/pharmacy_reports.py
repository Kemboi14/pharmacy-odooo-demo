...
class PharmacyDashboard(models.AbstractModel):
    ...
    def _get_branch_overview(self):
        ...
        active_branches = self.env['pharmacy.branch'].search_count([('active', '=', True)])
    ...
    def _get_sales_overview(self, today, month_start, year_start):
        ...
        sales_growth = ((mtd_sales - prev_month_sales) / prev_month_sales * 100) if prev_month_sales > 0 else 0
    ...
    def _get_inventory_overview(self):
        ...
        for lot in expiring_lots:
            ...
            expiring_30d_value += sum(quants.mapped(lambda q: q.quantity * q.product_id.standard_price))
    ...
    def _get_insurance_overview(self):
        ...
        approved_count = len(all_claims.filtered(lambda c: c.status in ['approved', 'partially_approved']))
