...
class PosOrder(models.Model):
    ...
    # Pharmacy fields
    branch_id = fields.Many2one('pharmacy.branch', 'Branch', required=False, tracking=True)
    prescription_id = fields.Many2one('pharmacy.prescription', 'Prescription', tracking=True)
    ...
    # Insurance amounts
    copay_amount = fields.Float('Co-pay Amount', compute='_compute_insurance_amounts', store=True)
    insurance_amount = fields.Float('Insurance Amount', compute='_compute_insurance_amounts', store=True)
    ...
    @api.depends('lines', 'lines.is_insurance_covered', 'lines.insurance_amount', 'lines.copay_amount')
    ...
    def create(self, vals_list):
        ...
        for order in orders:
            ...
            try:
                ...
                _logger.error(f"Failed to auto-create dispensing records for order {order.name}: {str(e)}")
            ...
            if order.is_insurance_sale:
                ...
                if claim and order.preauth_code:
                    ...
                    try:
                        ...
                        _logger.warning(f"Failed to auto-submit claim for order {order.name}: {str(e)}")
            ...
            for line in self.lines:
                if line.product_id.is_pharma_product:
                    self.env['pharmacy.dispensing'].create({
                        'pos_order_id': self.id,
                        'pos_order_line_id': line.id,
                        'patient_id': self.partner_id.pharmacy_patient_id.id if self.partner_id and self.partner_id.pharmacy_patient_id else False,
                        'product_id': line.product_id.id,
                        'lot_id': line.lot_id.id,
                        'quantity': line.qty,
                        'branch_id': self.branch_id.id,
                ...
                if line.product_id.is_pharma_product:
                    ...
                    if prescription_line:
                        ...
                        if check_result['allowed']:
                            ...
                            _logger.warning(f"Dispensing not allowed: {check_result['reason']}")
    ...
    def write(self, vals):
        ...
        if self.claim_id and self.claim_id.status != 'draft':
            if 'is_insurance_sale' in vals or 'insurer_id' in vals or 'plan_id' in vals:
                raise ValidationError(_('Cannot modify insurance details after claim submission'))
    ...
    def _prepare_invoice_vals(self):
        ...
        if self.is_insurance_sale:
            vals['narration'] = f"Insurance Sale - {self.insurer_id.name} - {self.plan_id.name}\nMember: {self.member_number}"
...
class PosOrderLine(models.Model):
    ...
    # Pharmacy fields
    lot_id = fields.Many2one('stock.lot', 'Batch/Lot', required=False,
        ...
        for line in self:
            if line.is_insurance_covered:
                if line.coverage_percentage:
                    line.insurance_amount = line.price_subtotal_incl * (line.coverage_percentage / 100)
                else:
                    line.insurance_amount = 0
                
                if line.copay_percentage:
                    line.copay_amount = line.price_subtotal_incl * (line.copay_percentage / 100)
                else:
        ...
        for line in self:
            if line.lot_id and line.product_id:
                if line.lot_id.product_id != line.product_id:
                    raise ValidationError(_('Selected batch does not match the product'))
