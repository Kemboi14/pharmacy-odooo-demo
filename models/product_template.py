...
class ProductTemplate(models.Model):
    ...
    dosage_form_id = fields.Many2one('pharmacy.dosage.form', 'Dosage Form', tracking=True)
    dosage_form = fields.Selection([
        ...
        ('patch', 'Patch'),
        ('other', 'Other')
    ], string='Dosage Form Category', compute='_compute_dosage_form', store=True)
    
    pack_size = fields.Integer('Pack Size', help="Tablets per strip, strips per box, etc.", tracking=True)
    
    # Regulatory fields
    is_prescription_required = fields.Boolean('Prescription Required', default=False, tracking=True)
    is_controlled_substance = fields.Boolean('Controlled Substance', default=False, tracking=True)
    ...
    ], 'Controlled Substance Schedule', default='unscheduled', tracking=True)
    requires_pharmacist_approval = fields.Boolean('Requires Pharmacist Approval', default=False, tracking=True)
    requires_id_capture = fields.Boolean('Requires ID Capture', default=False, tracking=True)
    
    # Storage and handling
    storage_conditions = fields.Text('Storage Conditions')
    interaction_warnings = fields.Text('Interaction Warnings')
    min_expiry_days = fields.Integer('Minimum Expiry Days', default=30,
    ...
    min_margin_percentage = fields.Float('Minimum Margin %', default=20.0,
                                         help="Minimum margin percentage for pricing")
    
    # Regulatory codes
    drug_registration_number = fields.Char('Drug Registration Number')
    ndc_code = fields.Char('NDC Code', help="National Drug Code")
    atc_code = fields.Char('ATC Code', help="Anatomical Therapeutic Chemical Classification")
    
    # Supplier information
    preferred_supplier_ids = fields.Many2many('res.partner', 'product_preferred_supplier_rel',
                                             'product_id', 'supplier_id', 'Preferred Suppliers')
    
    # Insurance pricing
    insurance_pricelist_ids = fields.Many2many('product.pricelist', 'product_insurance_pricelist_rel',
                                              'product_id', 'pricelist_id', 'Insurance Pricelists',
                                              help='Pricelists specific to insurance providers')
    
    # Tax configuration for Kenyan pharmacy
    supplier_taxes_id = fields.Many2many('account.tax', string='Supplier Taxes', 
                                         domain=[('type_tax_use', '=', 'purchase')])
    customer_taxes_id = fields.Many2many('account.tax', string='Customer Taxes',
    ...
    def _compute_is_pharma_product(self):
        for product in self:
            product.is_pharma_product = product.is_prescription_required or product.is_controlled_substance
    ...
    def check_sale_allowed(self, lot_id=None, quantity=1):
        ...
        if self.is_pharma_product:
            ...
            if lot_id:
                ...
                if lot.days_to_expiry < self.min_expiry_days:
                    result['warnings'].append(f'Batch expires in {lot.days_to_expiry} days (minimum: {self.min_expiry_days})')
        ...
        if self.is_controlled_substance:
            result['warnings'].append('Controlled substance - pharmacist verification required')
    ...
    def get_pricing_info(self, pricelist_id=None, quantity=1):
        ...
        """
        if not pricelist_id:
            pricelist_id = self.env['product.pricelist'].search([('company_id', '=', self.env.company.id)], limit=1)
        ...
        if price < min_price:
            result = {
                ...
                'margin_percentage': ((price - cost_price) / cost_price * 100) if cost_price > 0 else 0,
            }
        else:
            result = {
                'price': price,
    ...
    def get_display_info(self):
        ...
        return {
            ...
            'dosage_form': self.dosage_form_id.display_name if self.dosage_form_id else self.get_dosage_form_label(),
    ...
    def _inverse_dosage_form(self):
        for template in self:
            if template.dosage_form:
                dosage_form = self.env['pharmacy.dosage.form'].search([('name', '=', template.dosage_form)], limit=1)
                if dosage_form:
    ...
    def get_dosage_form_label(self):
        ...
        return dict(self._fields['dosage_form'].selection).get(self.dosage_form, self.dosage_form)
