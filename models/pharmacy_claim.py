...
class PharmacyClaim(models.Model):
    ...
    insurer_id = fields.Many2one('pharmacy.insurer', 'Insurer', required=True, tracking=True)
    plan_id = fields.Many2one('pharmacy.insurer.plan', 'Insurance Plan', required=True, tracking=True)
    ...
    # POS order reference
    pos_order_id = fields.Many2one('pos.order', 'POS Order', required=True, readonly=True)
    
    # Dates
    claim_date = fields.Date('Claim Date', required=True, default=fields.Date.today, tracking=True, index=True)
    ...
    approved_amount = fields.Float('Approved Amount', compute='_compute_amounts', store=True)
    rejected_amount = fields.Float('Rejected Amount', compute='_compute_amounts', store=True)
...
class PharmacyClaimLine(models.Model):
    ...
    claim_id = fields.Many2one('pharmacy.claim', 'Claim', required=True, ondelete='cascade')
    ...
    # Claim amounts
    claimed_amount = fields.Float('Claimed Amount', compute='_compute_claimed_amount', store=True)
    approved_amount = fields.Float('Approved Amount', default=0.0)
    rejected_amount = fields.Float('Rejected Amount', compute='_compute_rejected_amount', store=True)
    copay_amount = fields.Float('Co-pay Amount', compute='_compute_copay_amount', store=True)
