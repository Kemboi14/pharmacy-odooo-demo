...
class AccountMove(models.Model):
    ...
    # eTIMS fields
    etims_invoice_number = fields.Char('eTIMS Invoice Number', readonly=True, copy=False)
    ...
    etims_status = fields.Selection([
        ...
        ('rejected', 'Rejected'),
        ('cancelled', 'Cancelled'),
    ], 'eTIMS Status', default='draft', tracking=True)
    ...
    # Configuration
    etims_auto_submit = fields.Boolean('Auto Submit to eTIMS', related='company_id.etims_auto_submit', readonly=True)
    
    def action_submit_to_etims(self):
        """Submit invoice to KRA eTIMS system"""
        for invoice in self:
            if invoice.etims_status not in ['draft', 'rejected']:
                raise UserError(_('Invoice can only be submitted in Draft or Rejected status'))
    ...
    def _prepare_etims_invoice_data(self):
        ...
        for line in self.invoice_line_ids:
            ...
            line_data = {
                ...
                'tax_amount': line.price_subtotal * (line.tax_ids[0].amount / 100) if line.tax_ids else 0,
            }
            
            # Add pharmacy-specific fields if applicable
            if hasattr(product, 'is_prescription_required') and product.is_prescription_required:
        ...
        if self.pos_order_ids:
            ...
            invoice_data.update({
                ...
                'branch_name': pos_order.branch_id.name if pos_order.branch_id else '',
    ...
    def action_cancel_etims(self):
        ...
        try:
            ...
            if result.get('success'):
                ...
                raise UserError(_('eTIMS cancellation failed: %s') % result.get('error'))
        ...
        try:
            ...
            if result.get('success'):
                ...
                if status == 'approved' and self.etims_status != 'approved':
                    ...
                    self.write({
                        'etims_status': 'rejected',
                        'etims_error_message': result.get('data', {}).get('rejection_reason'),
                    })
                ...
                raise UserError(_('Failed to check eTIMS status: %s') % result.get('error'))
...
class ResCompany(models.Model):
    ...
    etims_api_key = fields.Char('eTIMS API Key', help='API key for eTIMS authentication')
    etims_tin_number = fields.Char('TIN Number', help='Tax Identification Number')
    etims_auto_submit = fields.Boolean('Auto Submit to eTIMS', default=False,
                                      help='Automatically submit invoices to eTIMS after posting')
    etims_environment = fields.Selection([
        ('sandbox', 'Sandbox'),
        ('production', 'Production'),
    ], 'eTIMS Environment', default='sandbox')
...
class EtimsReportWizard(models.TransientModel):
    ...
    ], 'Report Type', required=True, default='sales')
